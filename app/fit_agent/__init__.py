...
WEIGHTS = {"personal": 0.20, "personal_size": 0.30, "product": 0.15, "product_size": 0.20, "review": 0.15}
...
def analyze(connection, user_id, product, size, review_result):
    ...
    def metric(label, subset, weight, return_rate=False):
        n = len(subset)
        ...
        return {"label": label, "count": n, "returned": returned, "success_pct": success,
                "value": (round(returned / n * 100, 1) if n else None) if return_rate else success,
                "weight": weight, "is_return_rate": return_rate}

    metrics = [
        metric("개인 구매 성공률 · 동일 카테고리", personal, WEIGHTS["personal"]),
        metric("개인 선택 사이즈 성공률 · 동일 카테고리", personal[personal['size'] == size], WEIGHTS["personal_size"]),
        metric("상품 반품률", product_orders, WEIGHTS["product"], True),
        metric("상품 선택 사이즈 반품률", product_orders[product_orders['size'] == size], WEIGHTS["product_size"], True),
    ...
    reasons.append(f"선택 사이즈의 상품 구매 표본은 {metrics[3]['count']}건, 리뷰 표본은 {review_result['count']}건입니다.")
