...
st.caption("최근 30건 · UTC 시각 · 동일 request_id로 Review와 Fit 연결")
if data["agents"]:
    st.dataframe(pd.DataFrame(data["agents"]), hide_index=True, width="stretch")
else:
    st.info("아직 Agent 실행 기록이 없습니다.")
with st.expander("작업별 자원 로그 · 최근 50건"):
    if data["resources"]:
        st.dataframe(pd.DataFrame(data["resources"]), hide_index=True, width="stretch")
    else:
        st.caption("기록 없음")
with st.expander("측정 기준과 한계", expanded=True):
    ...
    - Fit은 내부 Review를 포함합니다. Agent 로그 2개와 자원 로그 1개가 생성되므로 Agent 시간을 합산하지 마세요.
